$(function () {
		...
		if (user_id && auth_key) {
			$.post(base_url + '/auth', {'user-id': user_id, 'auth-key': auth_key}, function (json_resp) {
				if (json_resp.error === false) {
					...
					$('#createModal .modal-body').html('Press close to refresh this page.');
	...
	$('#all-data').on('click', '.btn-view-stats', function () {
		var hidden_hash = $(this).closest('td').find('input').val();
		
		$.get(base_url + '/get-stats', {'hash': hidden_hash}, function(json_resp) {
			if (json_resp.error === false) {
				$('#statsModal h5.modal-title').html('Link Statistics');
				
				if (!$.fn.DataTable.isDataTable('#stats-table')) {
					$('#stats-table').DataTable({
						data: json_resp.stats,
						columns: [
							{title: "User Agent"},
							{title: "Request IP"},
							{title: "Accessed",
					...
					$('#stats-table').DataTable().clear().draw();
					$('#stats-table').DataTable().rows.add(json_resp.stats).draw();
				}
				
			} else {
				$('#statsModal h5.modal-title').html('There seems to be a problem...');
				$('#statsModal .modal-body').html(json_resp.message);
			}
	...
	$.ajaxSetup({
		headers: {'Authorization': "Basic " + $('input[name="token"]').val()}
	});
});